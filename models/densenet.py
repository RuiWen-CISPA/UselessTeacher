...
class AdvDenseNet(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.features(output)
        output = self.avgpool(output)
        embedding = output.view(output.size()[0], -1)
...
def advdensenet121(**kwargs):
    return AdvDenseNet(Bottleneck, [6,12,24,16], growth_rate=32, **kwargs)
